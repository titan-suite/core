...
const aionMainnet = new Aion(nodeAddress.aionMastery)
...
describe('Test AION class methods', () => {
  ...
  it('should send a signed transaction to a contract on the mainnet', async () => {
    ...
    mainnetContractInstance = new aionMainnet.web3.eth.Contract(
      abi,
      address
    )
    const data = await mainnetContractInstance.methods.setA(5).encodeABI()
    const estimate = await mainnetContractInstance.methods.setA(5).estimateGas()
  ...
  //   const compiled = await aionMainnet.compile(WithConstructorContract.sol)
  //   const abi: any = compiled[WithConstructorContract.name].info.abiDefinition
  //   let address: string = "0xa0c2d5954124eEEb55A862e20e925DB954C39d071876687a0E48F22297F54c48" // mainnetContractAddress
  //   mainnetContractInstance = new aionMainnet.web3.eth.Contract(
  //     abi,
  //     address
  //   )
